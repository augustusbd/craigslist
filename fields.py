...
class Fieldset():
	...
	def return_name_attributes(self):
		...
		if self.does_subfield_exist(self._field) != False:
...
def get_user_input_for_others(tags):
    ...
    for tag in tags:
        ...
        type_ = put_strings_together_from_list(type_)
...
def radio_input(tag):
    ...
    for radio in radios:
        ...
        title = radio.get_attribute_list('title')   # title information from tag
        title = m.put_strings_together_from_list(title)
        value = tag.get_attribute_list('value')     # value of radio button (to select)
        value = put_strings_together_from_list(value)
        
        radio_dict[str(pos)] = value                # store value of radio button

        print("Option #{0}: {1}".format(pos, title))    # gives radio button info
        print("Basically means: " + find_siblings_text(radio_tag))
